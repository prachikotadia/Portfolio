  ...
  return (
    ...
    >
      ...
      <div className={`transform transition-all duration-500 preserve-3d ${hoveredIndex === index ? 'rotateY(10deg) rotateX(5deg)' : ''}`}>
        <div className="absolute top-4 right-4 z-10 text-xl opacity-70 group-hover:opacity-100 
                      group-hover:scale-150 transition-all duration-500 group-hover:rotate-12">
          {project.emoji}
        </div>
        ...
        <div className="p-6 relative">
            ...
            {project.tags.map((tag, tagIndex) => (
              <span 
                ...
                className="px-2.5 py-0.5 text-xs rounded-full bg-primary/10 text-primary
                        group-hover:bg-primary/20 transition-colors duration-300 hover:scale-105 hover:bg-primary/30"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex justify-between items-center pt-2">
            <a 
              ...
              className="flex items-center text-sm font-medium hover:text-primary transition-all duration-300 hover:scale-110 animate-jelly"
              aria-label={`View code for ${project.title}`}
              onClick={() => handleProjectClick('code')}
            >
              <Github className="mr-1.5 h-4 w-4 transition-transform duration-300 hover:rotate-12" />
              Code
            </a>
            <a 
            ...
            >
              <ExternalLink className="mr-1.5 h-4 w-4 transition-transform duration-300 hover:rotate-12" />
              Live Demo
            </a>
          </div>
        </div>
